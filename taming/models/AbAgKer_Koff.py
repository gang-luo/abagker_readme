...
class AbAgKerTrainer(pl.LightningModule): 
    def __init__(self,
        ...
        self.koff_metrics = torchmetrics.MetricCollection({
            "koff_pearson": torchmetrics.PearsonCorrCoef(),
            "koff_spearman": torchmetrics.SpearmanCorrCoef(),
            "koff_rm2": RM2(),
            "koff_ci": CI(),
            "koff_mse": torchmetrics.MeanSquaredError(),
            "koff_rmse": torchmetrics.MeanSquaredError(squared=False),
        }, prefix="val/")
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        kd_pre, koff_pre, aux_dict = self.model(HLX,ex_HXL)

        if optimizer_idx == 0: 
            total_loss, log_dict = self.loss(self.train_type, kd_pre, koff_pre, kd, koff, aux_dict, optimizer_idx, split="train") 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        kd, koff, AbAgI = label[0], label[1], label[2]
        kd_pre, koff_pre, _ = self.model(HLX,extra_HXL)
        self.kd_metrics.update(kd_pre, kd)
        self.koff_metrics.update(koff_pre, koff)

    def validation_epoch_end(self,outputs):
        kd_metrics = self.kd_metrics.compute()
        koff_metrics = self.koff_metrics.compute()
        self.log_dict(kd_metrics, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(koff_metrics, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        
        current_monitor_value = koff_metrics.get(self.monitor, None)
    ...
    def configure_optimizers(self):
        ...
        for param in self.model.LightModel.parameters():
            param.requires_grad = False
        for param in self.model.feature_layer.parameters():
            param.requires_grad = False
        
        betas = (self.opt_config.get('beta1', 0.9), self.opt_config.get('beta2', 0.999))
        optimizer_groups = [
            {'params': self.model.koff_regressor.parameters(), 'lr': self.learning_rate}
        ]
        ...
        if self.opt_config.get('warm_up_steps') is not None:
            ...
            lr_scheduler = {
                ...
                'interval': 'step',
                'frequency': 1, # every step update the lr
            }
